...
export async function startTui(baseUrl, options = {}) {
    ...
    const askInput = (label) => new Promise((resolve) => {
        prompt.input(label, "", (_err, value) => resolve((value || "").trim()));
    });
    const askYesNo = (label) => new Promise((resolve) => {
    ...
    const runAction = async (index) => {
        ...
        try {
            if (index === 0) {
                const method = await askInput(preferAscii ? "Input login method: phone/email/qr" : "输入登录方式: phone/email/qr");
                if (method === "phone") {
                ...
                if (!scanPath) {
                    ...
                    const songs = await app.localScanner.scan(scanPath, (p) => {
                        const now = Date.now();
                        if (now - lastUiUpdate < 300 && p.current !== p.total)
                            return;
                        lastUiUpdate = now;
                ...
                const cloud = await app.cloudService.getCloudSongs(false);
                const diff = app.diffSyncService.buildDiff(local, cloud);
                const ok = await askYesNo(preferAscii
