...
export async function startTui(baseUrl) {
    ...
    const runAction = async (index) => {
        try {
            if (index === 0) {
                ...
                output.log(`差异：本地独有 ${diff.localOnly.length}，云盘独有 ${diff.cloudOnly.length}，精准匹配 ${diff.matchedExact.length}，模糊匹配 ${diff.matchedFuzzy.length}`);
            }
            else if (index === 5) {
                const local = app.cacheRepo.getLocalSongs();
                const cloud = await app.cloudService.getCloudSongs(false);
